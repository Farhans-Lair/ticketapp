-- V5 — COMPOSITE INDEXES + OPTIMISTIC LOCK COLUMN Implements analysis recommendations: Composite index on events(event_status, event_date)
...
-- events Public event listing: ORDER BY event_date WHERE event_status = 'published'
...
-- bookings Attendee list + organizer revenue: WHERE event_id = ? AND payment_status = 'paid'
...
-- Idempotency: prevents double-booking on duplicate verifyPayment calls Note: only adds if razorpay_order_id column exists (added in
CALL tv_add_index('bookings', 'uq_bookings_razorpay_order', '(razorpay_order_id)');

-- seats Seat lookup by event + seat number (pessimistic lock path)
